...
cat > "$ROOT/services/execution_service/app/update_consumer.py" <<'PY'
...
class OrderPersistenceRepository:
    ...
    def get_order_by_external_order_id(self, external_order_id: str) -> PersistedOrder | None:
        with self._postgres_client.cursor() as cur:
            cur.execute(
                """
                SELECT
...
text = path.read_text()
marker = "# Item 28 — Broker order update consumer"

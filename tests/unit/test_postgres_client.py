...
def build_settings(postgres_enabled: bool) -> Settings:
    return Settings(
        APP_ENV="local",
        APP_NAME="projectX",
        LOG_LEVEL="INFO",
...
def test_postgres_dsn_property() -> None:
    settings = build_settings(postgres_enabled=False)
    assert settings.postgres_dsn == "postgresql://projectx:changeme@localhost:5432/projectx"


def test_postgres_disabled_fetch_returns_none() -> None:
    client = PostgresClient(settings=build_settings(postgres_enabled=False))
    client.connect()
    value = client.fetch_one("SELECT 1")

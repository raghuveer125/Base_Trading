...
def test_cache_writer_methods_do_not_fail_when_redis_disabled() -> None:
    ...
    writer = IndicatorCacheWriter(
        settings=settings,
        redis_client=redis_client,
    )

    writer.cache_indicator(build_indicator_event())
    writer.cache_status(
        IndicatorEngineStatus(
            service="indicator_engine",
            mode="stub",
            connected=True,
            source_topic="md.bar.1m.closed",
            target_topic="md.indicator.1m",
            timeframe="1m",
            consumed_count=1,
            produced_count=1,
            last_processed_at=None,
            message="Indicator engine connected",
        )
    )